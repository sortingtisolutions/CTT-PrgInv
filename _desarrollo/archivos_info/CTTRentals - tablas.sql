...
CREATE TABLE 'cttapp_cire'.'ctt_budget' (
  ...
  'bdg_section' SMALLINT(1) DEFAULT 1 COMMENT 'Numero de seccion',
  'bdg_quantity' INT NULL COMMENT 'Cantidad de productos',
  'bdg_days_base' INT NULL COMMENT 'Días solicitados en renta',
  'bdg_days_cost' INT NULL COMMENT 'Días en cotización en renta',
...
SELECT
  ...
  pc.ver_id,
  ROW_NUMBER() OVER (
    partition by pr.prd_sku
    ORDER BY
      pr.prd_name asc
  ) AS num
FROM
  ctt_projects_content AS pc
...
WHERE
  (
    pd.pjtdt_prod_sku = 'Pendiente'
    OR LEFT(RIGHT(pd.pjtdt_prod_sku, 4), 1) = 'R'
  );
...
SELECT
  ...
  str_name,
  ser_id,
  DATE_FORMAT(sub_date_start, '%d/%m/%Y') AS sub_date_start,
  DATE_FORMAT(sub_date_end, '%d/%m/%Y') AS sub_date_end,
  sub_comments,
  pjtcn_days_base,
  pjtcn_days_trip,
  pjtcn_days_test,
  ifnull(prd_id, 0) AS prd_id,
  ifnull(sup_id, 0) AS sup_id,
  ifnull(str_id, 0) AS str_id,
  ifnull(sub_id, 0) AS sub_id,
  ifnull(sut_id, 0) AS sut_id,
  ifnull(pjtdt_id, 0) AS pjtdt_id,
  ifnull(pjtcn_id, 0) AS pjtcn_id,
  ifnull(cin_id, 0) AS cin_id
FROM
  ctt_vw_subletting;
