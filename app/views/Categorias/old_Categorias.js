    ...
    $.ajax({
        ...
        success: function (respuesta) {
            ...
            if (respuesta != 0) {
                ...
                // $(rowNode).attr('id', respuesta);
                // console.log($(rowNode));

                // get_quantity(respuesta);
                LimpiaModal();
                getAlmacenes();
            }
        },
...
function settingTable() {
    ...
    $('#CategoriasTable').DataTable({
        ...
        columns: [
            ...
            {data: 'catname', class: 'product-name'},
            {data: 'storename', class: 'product-name'},
...
function getCategoriasTable() {
    ...
    $.ajax({
        ...
        dataType: 'JSON',
        url: location,
        _success: function (dt) {
            let tabla = $('#CategoriasTable').DataTable();

            $.each(dt, function (v, u) {
                var btns = `
                    <button onclick="EditCategoria(${u.cat_id})" type="button" class="btn btn-default btn-icon-edit" aria-label="Left Align"><i class="fas fa-pen modif"></i></button>
                    <button onclick="ConfirmDeletCategoria(${u.cat_id},${u.cantidad})" type="button" class="btn btn-default btn-icon-delete" aria-label="Left Align"><i class="fas fa-times-circle kill"></i></button>
                    `;
                var qty = `<span class="toLink">${u.cantidad}</span>`;
    ...
    $.ajax({
        ...
        _success: function (respuesta) {
            ...
            respuesta.forEach(function (row, index) {
                ...
                //     '</td>' +
                //     "<td class='dtr-control text-center category'>" +
                //     row.cat_id +
                //     '</td>' +
                //     '<td class="catname">' +
                //     row.cat_name +
