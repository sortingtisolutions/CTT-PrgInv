...
function discountInsuredEvent() {
    $('.selectioninsured').unbind('click').on('click', function() {
        let elm = $(this);
...
function eventsAction() {
    ...
    $('.projectInformation').unbind('click').on('click', function() {
    ...
    $('.projectfinder').unbind('click').on('click', function() {
    ...
    $('.showColumns').unbind('click').on('click', function() {
        let rotate = $(this).attr('class').indexOf('rotate180');
        console.log(rotate);
    ...
    $('.menu-sections ul li').unbind('click').on('click', function() {
    ...
    $('.removeSection').unbind('click').on('click', function() {
        let id = $(this);
    ...
    $('.version__button').unbind('click').on('click', function() {
    ...
    $('#btnEditProject').unbind('click').on('click', function() {
        let pjtId = $('.projectInformation').attr('data-project');
        editProject(pjtId);
    });
    // Agrega nuevo proyecto
    $('#btnNewProject').unbind('click').on('click', function() {
        newProject();
    });

    // Agrega nueva cotización
    $('#newQuote').unbind('click').on('click', function() {
    ...
    $('.toPrint').unbind('on').on('click', function() {
    ...
    $('.inputSearch').unbind('keyup').on('keyup', function() {
        let id = $(this);
    ...
    $('.cleanInput').unbind('click').on('click', function() {
    ...
    $('.sidebar__comments .toComment').unbind('click').on('click', function() {
        showModalComments();
    });
...
function putDiscounts(dt) {
    $.each(dt, function(v, u) {
        let H = `<option value="${u.dis_discount}">${u.dis_discount *
            100}%</option>`;
        ...
        $.each(dt, function(v, u) {
            caret =
                firstVersion == u.ver_id
                    ...
                    : '';
            let H = `<li id="V${u.ver_id}" data-code="${u.ver_code}" data-discount="${u.ver_discount_insured}">
...
function selectorProjects(pjId) {
    $('.finder_list-projects ul li').unbind('click').on('click', function() {
        cleanQuoteTable();
        showButtonVersion('H');
        showButtonToPrint('H');
        showButtonToSave('H');
...
function selectCustomer() {
    $('.finder_list-customer ul li').unbind('click').on('click', function() {
...
function showListProducts(item) {
    ...
    $('#txtProductFinder').unbind('keyup').on('keyup', function() {
...
function putProducts(dt) {
    ...
    $.each(dt, function(v, u) {
        let H = `
            <tr data-indx ="${v}" data-element="${u.prd_sku}|${u.prd_name.replace(
            /"/g,
            ''
        )}|${u.sbc_name}">
...
function reOrdering() {
    $('tbody.sections_products').find('tr.budgetRow').each(function(index) {
        if (index >= 0) {
            $(this).find('i.move_item').attr('data-order', index + 1);
        }
    });
}
...
function fillBudgetProds(jsn, days) {
    ...
    let H = `
    <tr id="bdg${pds.prd_id}" data-sku="${pds.bdg_prod_sku}" data-insured="${pds.bdg_insured}" data-level="${pds.bdg_prod_level}" class="budgetRow">
        <th class="wclprod col_product product"><i class="fas fa-ellipsis-v move_item" data-order="0"></i><div class="elipsis" title="${prdName}">${prdName}</div><i class="fas fa-bars menu_product" id="mnu${pds.prd_id}"></i></th>
        ...
        <td class="wcldisc col_discount colbase discountBase" data-key="1"><i class="fas fa-caret-left selectioncell"></i><span class="discData">${parseFloat(
            pds.bdg_discount_base
        ) * 100}<small>%</small></span></td>
        <td class="wcldisc col_discount colbase discountInsu" data-key="${pds.bdg_insured}"><i class="fas fa-caret-left selectioncell"></i><span class="discData">${parseFloat(
        pds.bdg_discount_insured
    ) * 100}<small>%</small></span></td>
        ...
        <td class="wcldisc col_discount coltrip discountTrip" data-key="1"><i class="fas fa-caret-left selectioncell"></i><span class="discData">${parseFloat(
            pds.bdg_discount_trip
        ) * 100}<small>%</small></span></td>
        ...
        <td class="wcldisc col_discount coltest discountTest" data-key="1"><i class="fas fa-caret-left selectioncell"></i><span class="discData">${parseFloat(
            pds.bdg_discount_test
        ) * 100}<small>%</small></span></td>
...
function activeInputSelector() {
    ...
    $('.input_invoice').unbind('blur').on('blur', function() {
        updateTotals();
        showButtonToPrint('H');
        showButtonToSave('H');
    ...
    $('.selectionInput').unbind('click').on('click', function() {
        let id = $(this);
    ...
    $('.selectioncell').unbind('click').on('click', function() {
        let id = $(this);
    ...
    $('.menu_product').unbind('click').on('click', function() {
        let id = $(this);
...
function fillContent() {
    ...
    $('.textbox').unbind('focus').on('focus', function() {
...
function fillData(inx) {
    ...
    $('#saveProject.update').unbind('click').on('click', function() {
...
function newProject() {
    ...
    $('#txtProjectDepend').unbind('change').on('change', function() {
...
function actionNewProject() {
    $('#saveProject.insert').unbind('click').on('click', function() {
        let ky = validatorFields($('#formProject'));
        if (ky == 0) {
            let projId = $('#txtProjectIdEdt').val();
            let projName = $('#txtProjectEdt').val();
...
function fillComments(pjtId) {
    ...
    $('#newComment').unbind('click').on('click', function() {
