$.fn.gantt = function (options) {
    //let locale = options.locale ? options.locale : 'es-ES';
    ...
    $.each(tasks, function (index, task) {
        if (deps.indexOf(task.id.toString()) < 0 && task.date_start && task.date_end) {
            ...
            if (task.dep) {
                // Se existe atividades de depedência, passa para função que monta dependências
        ...
        if (tooltipShow) {
            $('#tbody_' + unic + ' > tr > .td-tasks').on('mouseover', function () {
                // Cria o tooltip ao passar o mouse na atividade
            ...
            $('#tbody_' + unic + ' > tr > .td-tasks').on('mousemove', function (e) {
                // Arrasta o tooltip de acordo com o mouse
                $('.tooltip-gantt').css('top', e.pageY + 10);
                $('.tooltip-gantt').css('left', e.pageX + 20);
            });

            $('#tbody_' + unic + ' > tr > .td-tasks').on('mouseout', function () {
                // Remove o tooltip ao tirar o mouse da atividade
    ...
    function checkCellColspan(type, task, qtdColspan, taskName, classTd, taskColor, labelT) {
        ...
        if (qtdColspan < 1000) {
            // Se quantidade colspan for menor de 1000
            cell += fillsCell(type, task, qtdColspan, originQtdColspan, taskName, classTd, taskColor, labelT, 'border-radius-full');
        }
        let countLoop = 1;
        while (qtdColspan > 1000) {
            // Enquanto quantidade colspan for maior que 1000 entra no looping até ficar menor que 1000
            if (qtdColspan > 1000) {
                let borderRadius = '';
                if (countLoop == 1) {
            ...
            if (qtdColspan < 1000) {
                // Se quantidade colspan atingir uma quantidade menor que 1000
            ...
            $.map(data, function (val, i) {
                ...
                if (val.id == id) {
                    if (moment(val.date_start, 'YYYY-MM-DD').isAfter(moment(lastDate, 'YYYY-MM-DD'))) {
                        ...
                        contentConflicts = `<p>${val.name} (ID: ${val.id}) - Início: ${moment(val.date_start, 'YYYY-MM-DD').format('DD/MM/YYYY')} Fim: ${moment(
                            val.date_end,
                            'YYYY-MM-DD'
                        ).format('DD/MM/YYYY')} esta conflitando</p>`;
                    }
                }
            });
        });
